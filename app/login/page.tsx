    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-xl border border-slate-200 p-8">
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full flex items-center justify-center gap-3 py-4 px-6 bg-white border border-slate-200 rounded-2xl shadow-sm hover:bg-slate-50 active:scale-[0.99] transition-all disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {isLoading ? (
          ...
          <span className="text-slate-700 font-semibold">
            {isLoading ? "جاري تسجيل الدخول..." : "تسجيل دخول باستخدام Google"}
          </span>
        </button>
      </div>
    </div>
